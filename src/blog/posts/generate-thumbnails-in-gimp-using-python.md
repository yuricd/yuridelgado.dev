...
If you already had to edit a photo to remove a pimple from your face, cut someone you don’t like, make your skin smoother or just created some art design, you probably had contact with a very famous software called Photoshop. 
...
Anyway, today we aren’t going to talk about Photoshop, but an [open-source](https://en.wikipedia.org/wiki/Open-source_software) and very nice option to it called GIMP. This guy is very powerful and lets you do amazing things, achieving great results as well as the Adobe software. BTW, I know people who prefer using GIMP instead of Photoshop. 🤷 

I consider GIMP a little more difficult to use but in the latest releases I felt they improved the UI and made things easier and cleaner. And the best part of GIMP is: it’s free and available for Windows, Linux and macOS!

## What we want to solve
Before starting to code, let’s think for a minute about the amount of information published to the web every hour. You probably follow some Youtubers that produce content frequently, maybe once, maybe twice a week, sometimes every day. Their videos usually require nice and attractive thumbnails to catch their audience's attention and bring views to the videos. If you have a huge channel and have a good profit from the channel, it may not be a problem because you can hire someone to make this work for you. However, if you are a new kid in the block you’ll probably spend some time in this process and, we all know, time is precious. So let’s create something to save Youtuber’s and content producer’s time and make everything with a blink of an eye.
...
I made a Google’s search in order to get thumbnail ideas and decide which one would be a good fit. After gathering all interesting options, I came out with this one GIMP file:
...
First of all, I have to give credits to the background images I used to write this post. All of them I got from [Unplash](https://unsplash.com?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) published by [Bailey Zindel](https://unsplash.com/@baileyzinde?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText), [Sharon McCutcheon](https://unsplash.com/@sharonmccutcheon?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText), [Andrea Davis](https://unsplash.com/@andreaedavis?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText), [Ania Archer](https://unsplash.com/@ania44?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) and [David Rangel](https://unsplash.com/@rangel?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText). The popping out pictures I took from Google and edited them, so I’m sure about the authors.
...
2. A brief text about the content we are sharing (three lines max. in our case)
...
Now, thinking about parameters we’ll read, I think of:
1. Background path as string
2. Picture path as string
3. Text lines as strings (1, 2 and 3)
4. Font face as string
5. Theme color as string

## Setting up GIMP
If you still don’t have, download and install [GIMP](https://www.gimp.org/).

We are going to use a feature of GIMP called [Python-Fu](https://docs.gimp.org/en/gimp-filters-python-fu.html) that allows us to write Python code as a plug-in to manipulate elements in the screen. The main procedures in GIMP are written in a functional [Lisp dialect](https://en.wikipedia.org/wiki/Lisp_(programming_language)) language called [Scheme](https://en.wikipedia.org/wiki/Scheme_(programming_language)) and GIMP exposes them in an API called [Procedure Database (PDB)](https://www.gimp.org/docs/python/procedural-database.html). This API will be our confederate while we develop the plug-in.

In order to make GIMP recognize a script as a plug-in, we should add the Python file within the GIMP’s plug-in folder. To check or change where GIMP folders are located in your machine, open GIMP and go to `Edit > Preferences > Folders`. In my machine they are located in `~/.config/GIMP/2.10`.

All the plug-ins have some lines dedicated to [register](https://docs.gimp.org/en/gimp-using-script-fu-tutorial-first-script.html) the plug-in. These lines will name, describe and address the plug-in to the GIMPs UI.

## Python Fu
As I mentioned before, GIMP let the use of Python to reach PDB. The PDB functions will let us create layers, add them to the image, load files, draw elements, position and reposition them and much more. Basically, everything you can perform in the GIMP’s UI can pretty much be achieved with Python Fu.

To know which function I should call for a certain operation, I constantly consulted the docs [here](https://www.gimp.org/docs/python/procedural-database.html) and [here](http://oldhome.schmorp.de/marc/pdb/index.html). To see the name, parameters description and whether it was deprecated, I used the Procedure Browser that can be accessed in `Filters > Python Fu > Console > Browse...`. A dialog box will open and searches can be made in the PDB using different fields.
...
The introduction is done, now let’s start to code.
...
The params we are using are the ones previously mentioned in [Defining what we want] section. We have to make GIMP aware about the type of param, variable name and a default value for the field. I’m using the same variable names in the  `thumb_generator` definition.
...
The PDB functions sometimes are confusing and full of parameters. Some functions do not take into account positional arguments, like `pdb.gimp_file_load(run_mode, filename, raw_filename)`, that you can provide `run-mode` or not. Therefore, I’ve chosen to wrap some operations in Python functions to make the use easier and clearer.
...
This function will be useful for debugging the execution whenever GIMP fail to open the plug-in dialog box or throw an exception.

When you have a general syntax issue in your code, GIMP will not even open the plug-in dialog box. Sometimes, it will prompt the error in the screen as I showed above or just log in the Error Console tab (to make it visible, click in the `arrow at the top right of GIMP’s UI > Add tab > Error Console`).

### load_file

Since we have the path of the background image (it should be in your filesystem), we just have to load and display it. This image will be the base of the whole art and we are going to see a reference to it along the code. Understand `image` as the “stage” of the art.
...
### add_layer
When you create a layer, you may want to add the layer to an image. In our scenario, understand “image” as the stage of the art. You can check the params in the documentation.
...
### load_image
This function will load an image into a layer (it was different from the `load_file` we did before) and return the layer, with the option of adding the layer to the `image`. 
...
Here I used the same “trick” as I did in `load_image` because we usually want to create it and add to the image. You can check the params in the documentation, but most of them are self-explanatory.
...
Since we have a custom color defined by the user, it's necessary to set the foreground to it.
...
The first task is to load the background image and display it a new window.
...
### Adding overlay
The overlay is just a layer with a white background and opacity that covers the size of the whole background image.
...
Here I considered the height of the bottom rectangle as 9% of the background size, create the layer, fill with the defined foreground color and translate it to the right side of the stage.
...
Here I performed some operations in this sequence:
Load the image
Calculate and resize the picture to have 47% of the background width keeping the proportions.
Translate the image layer to the right side of the stage.
Drop a smooth shadow behind the image. 
...
Text size must be also relative to the image. The first two lines calculate a 24% and 12% size using the image height as reference. The next two lines calculate the left and top margins. For these four sizes I tested visually and chose what looked better. Between each line, I had to recalculate the top margin to use the background as reference (you can think of a CSS relative position to the parent).

The first text line has a big font size and a margin of 1/7 of the total height. The second text line will have a background in the text, so I set the text color to white and the background color. Notice: I added a space in both sides of the text to create an internal padding in the final box. And the last line I just display the text in a “normal” font size.

The text section opens a vast set of options when it comes to usability and creativity. My version always considers the three lines to display the text in a good position. Users cannot provide some of them, it won’t throw an error. However, it’ll probably make the image weird due to empty lines or unalignment. So, there’s room to improve and make the display more dynamic according to the inputs.
